...
- [packages/ui/PRODUCT.md](../vendor/metacubexd/packages/ui/PRODUCT.md) defines the product and its
  users.
- [packages/ui/DESIGN.md](../vendor/metacubexd/packages/ui/DESIGN.md) defines the UI design system.
- [packages/agent/MANUAL.md](../vendor/metacubexd/packages/agent/MANUAL.md) contains real-kernel
  smoke tests that are intentionally outside CI.
- [README.md](../README.md) documents supported deployment forms.

## Monorepo Map

This is a pnpm 10 workspace with five workspaces:

| Workspace                          | Responsibility                                                                                            |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------- |
| `vendor/metacubexd/packages/ui`    | Nuxt 4/Vue 3 dashboard shared by every runtime form                                                       |
| `vendor/metacubexd/packages/agent` | Framework-neutral Control API, profile store, kernel supervisor, scheduler, and shared types              |
| `vendor/metacubexd/apps/server`    | Nitro all-in-one server that serves the UI and mounts the agent                                           |
| `vendor/metacubexd/apps/desktop`   | Electron shell, local control server, OS integration, privileged TUN helper, and bundled kernel packaging |
| `apps/desktop-deno`                | Deno Desktop shell and cross-platform packaging                                                           |

Do not move host-specific behavior into `vendor/metacubexd/packages/ui`. Put
reusable lifecycle, profile, and Control API behavior in
`vendor/metacubexd/packages/agent`; keep Docker/Nitro wiring in
`vendor/metacubexd/apps/server` and Electron/OS wiring in
`vendor/metacubexd/apps/desktop`.
...
- **Clash API** is Mihomo's `external-controller` HTTP/WebSocket surface. It owns
  ...
  Clash log data. UI access is centered on `vendor/metacubexd/packages/ui/composables/useApi.ts`,
...
- **Control API** is metacubexd's `/api/control/**` surface. It owns kernel
  ...
  goes through `vendor/metacubexd/packages/ui/composables/useControlApi.ts`; features are gated by
...
- Vue, Nuxt, VueUse, project composables, stores, utilities, constants, types,
  and components are auto-imported according to
  `vendor/metacubexd/packages/ui/nuxt.config.ts`.
...
in `vendor/metacubexd/packages/ui/i18n/locales/` aligned:
...
pnpm build:ui      # nuxt generate -> vendor/metacubexd/packages/ui/.output/public
...
pnpm generate      # build:ui, then copy the UI .output to root .output
pnpm typecheck     # run workspace typecheck scripts
pnpm lint          # runs available lint scripts; currently UI only and auto-fixes
```

Deno Desktop packages require renderer generation, kernel staging, and platform
packaging shown in `.github/workflows/desktop-build.yml`.
...
- UI unit specs live beside their areas under `**/__tests__/**/*.spec.ts`; UI
  browser-flow specs live in `vendor/metacubexd/packages/ui/e2e/`.
- Agent tests live in `vendor/metacubexd/packages/agent/src/**/*.test.ts` and use injected process,
  filesystem, fetch, and timer seams instead of a real kernel.
- Server tests live under `vendor/metacubexd/apps/server/**/__tests__/`.
- Electron Desktop tests live under
  `vendor/metacubexd/apps/desktop/src/**/__tests__/` and must not perform
  real elevation or OS changes.

Add regression coverage in the workspace that owns the behavior. Use
`vendor/metacubexd/packages/agent/MANUAL.md` only when real Mihomo or network behavior must be
...
- Do not hand-edit generated output: `.nuxt/`, `.nitro/`, `.output/`,
  `vendor/metacubexd/packages/ui/.output/`,
  `vendor/metacubexd/apps/server/.output/`,
  `vendor/metacubexd/apps/desktop/out/`, renderer, or dist directories.
- Do not hand-edit downloaded kernel artifacts in
  `vendor/metacubexd/apps/desktop/resources/`
